...
export const uploadMaterial = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (type === 'TEMPLATE') {
      ...
      **هام جداً:** استخرج كافة الأسئلة الممكنة التي تغطي جميع أفكار وصفحات الملف بالكامل دون استثناء (استخرج 100 سؤال على الأقل أو أكثر إن كان الملف يسمح بذلك). لا تكتفي بتقديم عينة صغيرة أبداً.
