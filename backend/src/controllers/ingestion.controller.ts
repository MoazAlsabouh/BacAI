...
export const uploadMaterial = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (type === 'TEMPLATE') {
      ...
      **الأنواع المسموحة للـ type هي فقط:** MCQ (أتمتة/اختيار من متعدد), ESSAY (أسئلة مقالية وشرح واستنتاج), PROBLEM_SOLVING (مسائل رياضية أو فيزيائية أو كيميائية شاملة), FILL_IN_BLANKS (املاء الفراغات).
      ...
      **هام جداً:** 
      ...
      - **الأنواع المسموحة للـ type هي فقط:** MCQ (أتمتة), ESSAY (مقالي أو استنتاج), PROBLEM_SOLVING (مسألة رياضية أو فيزيائية شاملة بطلبات متعددة), FILL_IN_BLANKS (املاء الفراغات).
