...
export const uploadMaterial = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (type === 'TEMPLATE') {
      ...
      **الأنواع المسموحة للـ type هي فقط:** 
      MCQ (أتمتة/اختيار من متعدد), 
      TRUE_FALSE (صح وخطأ), 
      MATCHING (مزاوجة/توصيل/صل بين), 
      ORDERING (ترتيب أحداث/كلمات), 
      FILL_IN_BLANKS (املاء الفراغات), 
      ESSAY (أسئلة مقالية، تعليل، شرح، استنتاج، ترجمة، موضوع تعبير), 
      PROBLEM_SOLVING (مسائل رياضية أو فيزيائية أو كيميائية شاملة), 
      COMPREHENSION (نص قراءة أو أبيات شعرية متبوعة بعدة أسئلة، ضع النص والأسئلة معاً في content), 
      DIAGRAM (سؤال يعتمد على رسمة، خريطة، أو جدول).
      ...
      مثال للناتج:
      {
        "sections": [
          ...
          { "title": "Read the following text then do the tasks below:", "type": "COMPREHENSION", "count": 1, "marksPerQuestion": 40, "topics": ["Reading Text", "Vocabulary"] }
        ]
      }`;
    } else {
      systemPrompt = `
      أنت خبير تعليمي في المناهج السورية (البكالوريا).
      المهمة: استخراج وتوليد أسئلة امتحانية من الملف المرفق لتكوين بنك أسئلة شامل.
      **هام جداً:** 
      1- استخرج كافة الأسئلة الممكنة التي تغطي جميع أفكار وصفحات الملف بالكامل دون استثناء (استخرج 100 سؤال على الأقل إن أمكن).
      2- **يجب** وضع كافة المتغيرات والمعادلات والرموز الرياضية بين علامتي $ (ضمن JSON Quotes).
      3- **القصائد والنصوص:** صحح أي أخطاء مطبعية في النصوص المستخرجة، مثل الكلمات المتلاصقة ببعضها (تأكد من وجود مسافات صحيحة بين الكلمات في أبيات الشعر والنصوص).
      - **الأنواع المسموحة للـ type هي فقط:** MCQ, TRUE_FALSE, MATCHING, ORDERING, FILL_IN_BLANKS, ESSAY, PROBLEM_SOLVING, COMPREHENSION, DIAGRAM.
      - **مواضيع السؤال (topics):** استنتج مواضيع السؤال بدقة لتتطابق مع قوالب الامتحانات.
      
      المخرج JSON مصفوفة من الأسئلة، إليك أمثلة:
      [ 
        { "type": "PROBLEM_SOLVING", "difficulty": 4, "content": "نص المسألة...", "topics": ["دراسة تغيرات"], "options": null, "correctAnswer": null, "rubric": "توزع الدرجات..." },
        { "type": "COMPREHENSION", "difficulty": 3, "content": "اقرأ الأبيات الآتية لفلان ثم أجب:\nالبيت الأول\nالبيت الثاني\n\nالأسئلة:\n1. استخرج شعوراً عاطفياً.\n2. أعرب الكلمة.", "topics": ["قصيدة كذا", "شعر"], "options": null, "correctAnswer": null, "rubric": "1. الشعور: حزن. 2. الإعراب: فاعل مرفوع" } 
      ]
      `;
    }
